...
export function renderOverview(pptx: PptxGenJS, lecture: LectureDocument): void {
  ...
  slide.addText(lecture.overview.title || 'A sequence for learning', {
    x: OVERVIEW_LAYOUT.TITLE_X, y: OVERVIEW_LAYOUT.TITLE_Y,
    w: OVERVIEW_LAYOUT.TITLE_W, h: OVERVIEW_LAYOUT.TITLE_H,
  ...
  if (richTextToPlain(lecture.overview.introduction)) {
    slide.addText(richTextRuns(lecture.overview.introduction), {
      x: OVERVIEW_LAYOUT.LEFT_COL_X, y: OVERVIEW_LAYOUT.INTRO_Y,
      w: OVERVIEW_LAYOUT.LEFT_COL_W, h: OVERVIEW_LAYOUT.INTRO_H,
  ...
  const rowH = Math.min(0.68, 3.55 / sectionCount);
  lecture.sections.forEach((section, index) => {
    const y = 2.82 + index * rowH;
