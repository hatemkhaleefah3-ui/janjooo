...
import type { ImportedImage, LectureDocument, LectureSlide } from '../schema/lecture-types';
...
const reusedReferenceImage = sampleImages['img-mitochondria'];
const intentionalReuseImages: Record<string, ImportedImage> = {
  'img-glycine-synthesis': reusedReferenceImage,
  'img-glycine-receptor': reusedReferenceImage,
  'img-catecholamines': reusedReferenceImage,
};
...
function aminoAcidLecture(): LectureDocument {
  const glycineSlides: LectureSlide[] = [
    ...
    imageTopic('g2', 'Glycine Synthesis', 'img-glycine-synthesis', 'Glycine is synthesized from serine by SHMT.', 'p2'),
    ...
    imageTopic('g5', 'Glycine Receptor Structure', 'img-glycine-receptor', 'The glycine receptor is a ligand-gated chloride channel.', 'p5'),
  ...
  const phenylalanineSlides: LectureSlide[] = [
    ...
    imageTopic('f4', 'Tyrosine to Catecholamines', 'img-catecholamines', 'Tyrosine is converted to dopamine, norepinephrine, and epinephrine.', 'p11'),
...
describe('regression: amino acid metabolism deck (issue #22)', () => {
  ...
  it('still renders the never-imported image slot as a labeled placeholder, not a silently blank slide', async () => {
    ...
    const result = await generateLecturePptx(lecture, intentionalReuseImages);
  ...
  it('supports the same imported image being intentionally reused across multiple slots', async () => {
    const lecture = aminoAcidLecture();
    const result = await generateLecturePptx(lecture, intentionalReuseImages);
    // The slots are unique, but each explicitly points to the same user-selected image bytes.
    expect(new Set(Object.values(intentionalReuseImages).map((image) => image.dataUrl)).size).toBe(1);
    for (const slotId of Object.keys(intentionalReuseImages)) {
      expect(result.warnings.some((warning) => warning.includes(`"${slotId}"`))).toBe(false);
    }
  });
});
